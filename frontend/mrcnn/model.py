...
def log2_graph(x):
    ...
    return tf.math.log(x) / tf.math.log(2.0) #changed
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    positive_count = tf.shape(positive_indices)[0] #changed
    # Negative ROIs. Add enough to maintain positive:negative ratio.
    r = 1.0 / config.ROI_POSITIVE_RATIO
    negative_count = tf.cast(r * tf.cast(positive_count, tf.float32), tf.int32) - positive_count
    negative_indices = tf.random.shuffle(negative_indices)[:negative_count] #changed
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    if config.DETECTION_MIN_CONFIDENCE:
        ...
        keep = tf.sparse.to_dense(keep)[0]  # Fixed from `tf.sparse_tensor_to_dense`
...
class MaskRCNN():
    ...
    def compile(self, learning_rate, momentum):
        ...
        for name in loss_names:
            layer = self.keras_model.get_layer(name)
            if any(layer.output is loss for loss in self.keras_model.losses):
                continue
            loss = (
                tf.reduce_mean(layer.output, keepdims=True)
                * self.config.LOSS_WEIGHTS.get(name, 1.)
